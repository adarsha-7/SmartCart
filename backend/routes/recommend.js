        ...
        try {
            ...
            if (recommendations.length < 3 && useFallback) {
                ...
                const categoryRecommendations =
                    await getCategoryBasedRecommendations(
                        productId,
                ...
                const newResults = categoryRecommendations.filter(
                    (p) => !existingIds.has(p.id)
                );

                recommendations = [...recommendations, ...newResults].slice(
                    0,
                    limit
                );
                method = "hybrid";
                console.log(
                    `Combined recommendations: ${recommendations.length} total products`
                );
            }
        } catch (error) {
            ...
            if (useFallback) {
                ...
                recommendations = await getCategoryBasedRecommendations(
                    productId,
                    limit
                );
...
router.get("/products/:productId/advanced", async (req, res) => {
    try {
        ...
        try {
            ...
            if (
                useFallback === "true" &&
                recommendations.length < parseInt(fallbackThreshold)
            ) {
                const categoryRecommendations =
                    await getCategoryBasedRecommendations(
                        productId,
    ...
    try {
        ...
        await Promise.all(
            productIds.map(async (productId) => {
                try {
                    ...
                    console.error(
                        `Batch recommendation error for product ${productId}:`,
                        error
                    );
                    batchResults[productId] = {
                        success: false,
        ...
        return res.status(200).json({
            ...
            summary: {
                ...
                successful: Object.values(batchResults).filter((r) => r.success)
                    .length,
                failed: Object.values(batchResults).filter((r) => !r.success)
                    .length,
            },
        });
    } catch (error) {
...
router.get("/analytics", async (req, res) => {
    try {
        ...
        const [totalProducts, totalCategories, totalSubCategories] =
            await Promise.all([
        ...
        return res.status(200).json({
            ...
            endpoints: {
                ...
                advancedRecommendations:
                    "/recommendations/products/:productId/advanced",
